...
\paragraph{Experiment 4: Budget-constrained allocation.}
Under the constraint $K_2+K_3=C$, the OA theory predicts that pure pairwise
coupling ($K_3=0$) maximizes $r^*$.  Numerical simulations at finite $N$,
however, show that moderate $K_3>0$ allocation can improve the
\emph{basin probability}---confirming that the mixed-allocation advantage
reported by \citet{Muolo2025} is a genuine finite-size effect not captured
by the mean-field theory.

\paragraph{Experiment 4b: Locking order.}
The correlation between $|\omega_i|$ and lock-on time reveals the
frequency-selection mechanism of higher-order coupling.  For $K_3<0$, the
correlation is strong ($\rho=-0.55$): oscillators with frequencies near
zero lock first, preserving the classical frequency-ordered entrainment.
For $K_3>0$, the correlation weakens to $\rho=-0.13$, indicating that
three-body cooperativity overrides frequency proximity---a ``winner-takes-all''
effect where initial phase alignment matters more than natural frequency.
This provides mechanistic insight into why $K_3>0$ narrows the basin
(sensitive to initial conditions) while strengthening the locked state.
...
\paragraph{Experiment 6: Chimera states (negative result).}
A systematic search for chimera states (coexistence of synchronized and
desynchronized subpopulations) across 15 $(K_2,K_3)$ configurations with
$N=500$ found no chimera signatures, even for large $|K_3|$.  This is
consistent with the theoretical expectation that chimera states require
broken coupling symmetry (e.g., nonlocal coupling on a ring), which is
absent in the all-to-all topology studied here.  Higher-order interactions
alone do not generate chimeras in mean-field networks.

\paragraph{Experiment 7: Frequency distribution shape effects.}
Comparing Gaussian, Lorentzian, and uniform frequency distributions at
$N=200$, $\sigma=1$: (i) the measured $\Kc$ differs across distributions
(Gaussian: $1.28$, Lorentzian: $1.30$, uniform: $1.81$), confirming that the
exact $\Kc$ depends on $g(0)$ as predicted; (ii) the $K_3$-dependence of
$\Kc$ is weak ($<10\%$ across $K_3\in[-1,1]$), consistent with our
\Cref{prop:onset-independence} that onset is $K_3$-independent at leading
order; (iii) the Lorentzian shows the smallest $K_3$ sensitivity
($\Delta\Kc=0.025$), validating that OA is most accurate for this
distribution.
...
Our analysis reveals a fundamental asymmetry between cooperative ($K_3>0$)
and competitive ($K_3<0$) higher-order interactions:
\begin{itemize}
  \item \textbf{$K_3>0$}: ``Elite synchronization''---stronger phase locking
    ($r^*\uparrow$) but harder to achieve (basin$\downarrow$), with
    potentially explosive transitions.
  \item \textbf{$K_3<0$}: ``Democratic synchronization''---weaker but more
    robust synchrony ($r^*\downarrow$, basin$\uparrow$), always continuous
    transitions on the OA manifold.
\end{itemize}
This asymmetry suggests that biological systems requiring robust,
moderate-amplitude synchrony (e.g., cortical oscillations) may preferentially
employ competitive higher-order coupling, while systems requiring strong
phase locking (e.g., cardiac pacemakers) benefit from cooperative coupling.
Numerically, the $K_3<0$ regime additionally supports multi-cluster
states---a beyond-OA phenomenon requiring multi-population extensions.
...
\begin{enumerate}
  \item $K_3^{\mathrm{eff}}\ge 0$ always: time delay can only induce
    \emph{positive} three-body coupling.  The $K_3<0$ half of our phase diagram
    is inaccessible to purely delayed systems.
  \item The physically reachable region in the $(K_2^{\mathrm{eff}},
    K_3^{\mathrm{eff}})$ plane is a triangle bounded by $K_3=0$ and
    $K_3=(K_2-K_2^{\mathrm{eff}})/2$.
  \item The explosive synchronization line $K_3=K_2$ (\Cref{thm:explosive})
    lies outside this triangle, explaining why explosive transitions are
    \emph{not} typically observed in time-delayed Kuramoto systems.
\end{enumerate}

\subsection{Limitations and four-body effects}

Our analysis truncates the interaction hierarchy at three-body terms.
Numerical experiments (see supplementary \texttt{K4\_test.json}) show that
adding a four-body coupling $K_4$ at $\Order(\tau^2)$ in the time-delay
expansion produces a $\sim$12\% variation in $r_1$ for $K_4\in[-1,1]$ at
$N=50$.  While this effect diminishes with increasing $N$ (the four-body term
scales as $N^{-3}$), it suggests that for applications where quantitative
accuracy matters---particularly in small networks---the four-body contribution
should not be neglected.

Additionally, the Ott--Antonsen reduction assumes all-to-all coupling.
For sparse networks or hypergraphs, the mean-field approximation breaks down,
and network topology introduces additional degrees of freedom that our
framework does not capture.  Extending the analysis to structured
networks~\citep{Muolo2025} remains an important direction.
...
\begin{enumerate}
  ...
  \item Three analytically derived critical manifolds that reconcile the four
    major contradictory studies (2020--2025).
  \item Discovery of a re-entrant synchronization region.
  \item A $\sigma$-scaling law (\Cref{prop:sigma-scaling}) showing phase
    boundaries are self-similar, answering the open question of
    \citet{Muolo2025}.
  \item Explicit mapping $K_3^{\mathrm{eff}}=K_2^2\tau/4$ from time-delayed
    to higher-order models, delineating the physically reachable region.
