...
quantitatively, including a re-entrant synchronization region for Gaussian
distributions where increasing $K_3$ first creates then destroys synchrony.
\end{abstract}
...
\begin{remark}[Lorentzian vs.\ Gaussian]
  ...
  \eqref{eq:r-dynamics}---including $r^*$, basin boundaries, and re-entrant
  loci---are exact only for the Lorentzian.  For the Gaussian, the OA
  prediction \emph{systematically underestimates} the synchronized order
  parameter $r^*$ by $0.10$--$0.27$ in the supercritical regime (see
  \texttt{gaussian\_selfconsistent.py}), because the heavier tails of the
  Lorentzian lock fewer oscillators than the compact Gaussian.  Nevertheless,
  the \emph{qualitative} structure of the phase diagram---including the
  existence and topology of the onset, explosive, basin, and re-entrant
  boundaries---is robust across distributions, and our analytical predictions
  provide correct scaling relations even when the quantitative $r^*$ values
  require Gaussian corrections~\citep{OttAntonsen2008}.
\end{remark}
...
\begin{figure}[t]
  ...
  \caption{\textbf{Left}: Analytical $K_2\times K_3$ phase diagram from OA
    ...
    ($K_3\!=\!K_2$, diagonal), and re-entrant boundaries.
...
\paragraph{Experiment 9: Re-entrant region.}
Our OA theory predicts re-entrant synchronization for $K_2$ slightly above
$\Kc$ when $K_3$ exceeds the explosive boundary $K_3\approx K_2$.
In the $20\times 20$ scan with $K_3\in[-2,2]$, $r^*$ increases monotonically
with $K_3$---the re-entrant drop is not observed because $K_3^{\max}=2$ does
not extend far enough into the explosive regime.  However, the hysteresis
experiment (Exp.\ 5) at $K_3=1.5$ shows hallmarks of first-order transition
(gap $0.41$), and the critical slowing data (Exp.\ 2) at $K_3=1.5$ shows
an abrupt relaxation time spike followed by collapse---both signatures
consistent with the onset of subcritical dynamics that would produce
re-entrance at larger $K_3/K_2$ ratios.  Targeted verification with
$K_3\in[0,6]$ at $K_2\approx\Kc$ is left to future work.
...
(4)~re-entrant synchronization confirmed for Gaussian distributions;
